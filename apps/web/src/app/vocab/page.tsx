...
export default function VocabPage() {
  ...
  useEffect(() => {
    fetch("/api/vocab")
      ...
      .catch((err) => setError(err instanceof Error ? err.message : "Failed to load vocab"));
  }, []);

  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-6 py-10">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Vocab bank</h1>
      ...
      {items && items.length === 0 && (
        <p className="text-sm text-neutral-500">
          No words yet — words you encounter in a learning session will show up here automatically.
        </p>
      )}
          ...
          return (
            <li key={item.id} className="flex items-center justify-between py-3">
              ...
              <div className="flex flex-col items-end text-xs text-neutral-400">
                <span>Seen {item.timesEncountered}×</span>
                <span className={due ? "font-medium text-neutral-900" : ""}>
                  {due ? "Due now" : `Due ${new Date(item.srs.dueAt).toLocaleDateString()}`}
                </span>
              </div>
            </li>
          );
